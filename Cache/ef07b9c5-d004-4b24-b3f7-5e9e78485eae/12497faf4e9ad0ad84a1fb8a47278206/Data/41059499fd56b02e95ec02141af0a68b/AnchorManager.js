...
exports.AnchorManager = void 0;
const Anchor_1 = require("Spatial Anchors/Anchor");
const AnchorModule_1 = require("Spatial Anchors/AnchorModule");
...
class AnchorManager {
    constructor() {
        this.logger = Logging_1.LoggerVisualization.createLogger("AnchorManager");
        this.log = this.logger.log.bind(this.logger);
        this.currentCuedSelectArea = null;
        this.nextCuedSelectArea = () => null;
        this.onAreaAnchorFoundEvent = new Event_1.default();
        this.onAreaAnchorFound = this.onAreaAnchorFoundEvent.publicApi();
        ...
        for (let i = 0; i < sceneObjectCount; i++) {
            ...
            if (sceneObject.name === "AnchorModule") {
                this.anchorModule = sceneObject.getComponent(AnchorModule_1.AnchorModule.getTypeName());
            }
        }
    }
    static getInstance() {
    ...
    cueSelectArea(areaId, afterSelect) {
        this.nextCuedSelectArea = async () => {
    ...
    selectArea(areaId, afterSelect) {
        this.cueSelectArea(areaId, afterSelect);
    }
    async createAnchorSession(areaId) {
        let options = new AnchorSession_1.AnchorSessionOptions();
        options.area = areaId;
    ...
    followSession(session) {
        ...
        session.onAnchorNearby.add((anchor) => {
            ...
            this.anchorFoundUnsubscribes.push(anchor.onFound.add(() => {
                this.currentAnchor = anchor;
                this.log(`AnchorManager: onAnchorFound: ${this.currentAnchor.toWorldFromAnchor.column3} and id: ${this.currentAnchor.id}`);
     ...
     */
    createAreaAnchor(anchorPosition, anchorRotation) {
        this.isCreatingNewAnchor = true;
        for (let i = 0; i < this.anchorFoundUnsubscribes.length; i++) {
            this.anchorFoundUnsubscribes[i]();
        }
        this.anchorFoundUnsubscribes = [];
        this.log(`AnchorManager: createAnchor w/ position ${anchorPosition} and rotation ${anchorRotation}`);
        this.currentAnchor = undefined;
        this.anchorSession
            .createWorldAnchor(mat4.compose(anchorPosition, anchorRotation, vec3.one()))
            .then(async (anchor) => {
            this.log(`AnchorManager: createAnchor success:${anchor.toWorldFromAnchor.column3} and id: ${anchor.id}}`);
            if (this.anchorSession) {
            ...
            .then(async (anchor) => {
            this.log(`AnchorManager: saveAnchor success:${anchor.toWorldFromAnchor.column3} and id: ${anchor.id}}`);
            ...
            if (anchor.state === Anchor_1.State.Found) {
                this.currentAnchor = anchor;
                this.log(`AnchorManager: immediately found:${this.currentAnchor.toWorldFromAnchor.column3} and id: ${this.currentAnchor.id}}`);
                this.onAreaAnchorFoundEvent.invoke(this.currentAnchor);
            }
            else {
                // anchor.onFound.add(() => {
     ...
     * @param anchorRotation - the desired world rotation of the area's mono-anchor.
     */
    updateAreaAnchor(anchorPosition, anchorRotation) {
        this.currentAnchor.toWorldFromAnchor = mat4.compose(anchorPosition, anchorRotation, vec3.one());
        this.log(`AnchorManager: updateAnchor: ${this.currentAnchor.toWorldFromAnchor}`);
